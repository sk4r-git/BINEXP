...
let run_ghidra ?(temp_dir = "/dev/shm") ?cache ~runner exe =
  ...
  let dir = base ^ ".d" in (*and len = String.length base in*)
  let include_path =
    (* List.find
      (fun path -> Sys.file_exists (path ^ "/ghidra_export.java"))
      Runtime.Sites.utils *)
    "~/Bureau/BINSEC_MOD/binsec45/src/ghidra/"
  in
  let command =
    Format.sprintf
  ...
  (* Basic_types.String.Set.iter (fun f ->
      let ic = open_in (Printf.sprintf "%s/%s" temp_dir base) in
      Printf.printf "%s/%s\n" temp_dir f;
      Printf.printf "1.5\n";
...
let import () =
  ...
  | Some path, None ->(Printf.printf "PATH + NONE\n"; parse_cache ~path)
  | cache, Some runner ->(Printf.printf "CACHE + RUNNER\n";
