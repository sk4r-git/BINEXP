...
module State = struct
  ...
  let assign ?(wild = false) name var_type value state =
    (* Printf.printf "\nin smt_symbolic.assign\n"; *)
    let open Formula in
    (* let info_key = 
      S.fold (fun key _value _acc -> key) state.var_infos "" in 
    let info_val = 
      S.fold (fun _key value _acc -> value) state.var_infos in 
    let info_acc = 
      S.fold (fun _key _value acc -> acc) state.var_infos "" in 
    let i_val2 = match info_val with
    | (x, _) -> x in *)

    (match S.find name state.var_infos with
    (* | _, vtype when vtype <> var_type && (String.get name 0) <> (String.get "%" 0) ->
