...
let pp ppf t =
  ...
  |> List.iter (fun (addr, instr) ->
         ...
         fprintf ppf "@[<h>%2d: %a@]@ " addr
           ...
           instr;
           match instr with            
           | Dba.Instr.Assign (_lval, _expr, _)->  fprintf ppf "  Assign \n" 
           | Dba.Instr.SJump (_, _)->  fprintf ppf "  Sjump \n" 
           | Dba.Instr.DJump (_, _)->  fprintf ppf "  Djump \n" 
           | Dba.Instr.If (_, _, _)->  fprintf ppf "  If\n "
           | Dba.Instr.Stop (_)->      fprintf ppf "  Stop \n"
           | Dba.Instr.Assert (_, _)-> fprintf ppf "  Assert \n"
           | Dba.Instr.Assume (_, _)-> fprintf ppf "  Assume \n" 
           | Dba.Instr.Nondet (_, _)-> fprintf ppf "  Nondet \n" 
           | Dba.Instr.Undef (_, _)->  fprintf ppf "  Undef \n"
           );
