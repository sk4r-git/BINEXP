# 1 "src/disasm/ppc64/ppc64ToDba.enabled.ml"
(**************************************************************************)
...
module Statistics = struct
  ...
  let pp ppf t =
    Format.fprintf ppf
      "@[<v 0>PPC decoding statistics@ ----@ Decoded (unique): %d (%d)@ Failed \
...
let find key kvs =
  ...
  with Not_found ->
    Ppc64_options.Logger.fatal "Decoder message has no %s field. Aborting." key
...
let to_block addr_instr_list =
  (* Blocks returned by Unisimi's PPC decoded are not necessarily ordered.
...
let parse_result s =
  Ppc64_options.Logger.debug ~level:1 "@[<v 0>Parsing %s@]" s;
  let open Lexing in
  let lexbuf = from_string s in
  try
...
let decode addr bytes = Ppc64dba.decode ~addr bytes |> parse_result
...
let decode reader addr =
  ...
  Ppc64_options.Logger.debug ~level:5 "@[%a@]" Dhunk.pp (snd res);
  Ppc64_options.Logger.debug ~level:3 "@[%a@]" show_stats ();
  res
...
Disasm_core.register_decoder (Machine.ppc64 Machine.BigEndian) cached_decode
  (fun x -> x)
