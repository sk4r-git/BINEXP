    ...
    Disasm_core.fold
      (fun cfg wlst inst set ->
        ...
        Virtual_address.Set.iter
          (Instr_cfg.add_edge_a cfg inst.Instruction.address)
          set;
        let block_callees = Dhunk.callees inst.Instruction.dba_block in
        Virtual_address.Set.iter
          (fun vaddr -> H.add callees vaddr ())
          block_callees;
        let set =
          Virtual_address.Set.filter (fun ins -> not (H.mem htbl ins)) set
        in
...
let run () =
  let ventry =
    ...
    | None ->
        ...
        |> Loader.Img.entry |> Virtual_address.create
  in
